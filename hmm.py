...
outer_divs=outerd=soup1.select('#ProductGrid-hnc082ysu9 > div > div > div > div > ul > li.ProductGridItem__itemOuter__KUtvv')#ProductGrid-hnc082ysu9 > div > div > div > div > ul > li:nth-child(1) > div.ProductGridItem__item__IkSDt.ProductGridItem__item-with-best-seller__JKmOp > div.ProductGridItem__itemInfo__wl2YN > div.ProductGridItem__itemInfoChild__hUHB0
#add code to find all the required elements in the first item, except raise exception
...
for i,div in enumerate(outer_divs):
    ...
    try:mrp=div.select('span > span > span > span.Price__whole__mQGs5')[1].text
    except:mrp=price
